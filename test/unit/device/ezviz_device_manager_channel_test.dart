...
void main() {
  ...
  test('boolean-return calls and list/map paths', () async {
    ...
    expect(
      await EzvizDeviceManager.addDevice(deviceSerial: 'D', verifyCode: ''),
      isTrue,
    );
    expect(await EzvizDeviceManager.deleteDevice('D'), isTrue);
    expect(
      await EzvizWifiManager.startConfigWifi(
        deviceSerial: 'D',
        ssid: 'S',
        password: 'P',
      ),
    ...
    final cloud = await EzvizDeviceManager.searchCloudRecordFiles(
      deviceSerial: 'D',
      startTime: 1,
      endTime: 2,
    );
    ...
    final dev = await EzvizDeviceManager.searchDeviceRecordFiles(
      deviceSerial: 'D',
      startTime: 1,
      endTime: 2,
    );
